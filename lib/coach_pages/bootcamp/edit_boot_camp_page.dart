...
class EditBootCampPage extends StatefulWidget {
  final BootCampDetails? bootCampDetails;

  const EditBootCampPage({Key? key, required this.bootCampDetails})
      : super(key: key);
  @override
  _EditBootCampPageState createState() => _EditBootCampPageState();
}

class _EditBootCampPageState extends State<EditBootCampPage> {
  ...
  initAll() {
    try {
      ...
      nameController =
          TextEditingController(text: "${bootCampDetails!.bootCampName}");
      desController =
          TextEditingController(text: "${bootCampDetails!.description}");
      priceController = TextEditingController(text: "${bootCampDetails!.price}");

      currentLocation = LatLng((bootCampDetails!.lat ?? 0.0) / 1.0,
          (bootCampDetails!.lon ?? 0.0) / 1.0);
      bookingAddress = "${bootCampDetails!.location}";

      selectedBookingDate = toBootCampBookingDates(bootCampDetails!);
      // print("final......$selectedBookingDate");
                    ...
                    Expanded(
                      child: InkWell(
                        onTap: () async {
                          ...
                          var cc = LatLng((d.lat ?? 51.5074) / 1.0,
                              (d.lon ?? 0.1278) / 1.0);
  ...
  createNewBootCamp(BuildContext context) async {
    ...
    try {
      GeneralResponse b = await updateBootCamp(
        bookingId: "${bootCampDetails!.id}",
