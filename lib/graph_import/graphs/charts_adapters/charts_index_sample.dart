...
// import 'package:graph_test/app.dart';
import 'chart_data_model.dart';
import 'week_data_chart.dart';
import 'month_data_chart.dart';
import 'year_data_chart.dart';
...
class Charts extends StatelessWidget {
  ...
  List<WeeklyStatsDataModel>? weeklyData = [
  ...
  List<StatsDataModel>? monthlyData = [
  ...
  List<StatsDataModel>? yearlyData = [
  ...
  Charts(
    ...
    switch (period) {
      case Period.WEEKLY:
        graph = WeekDataChart(
          _createWeeklySeriesList(weeklyData!),
          animate: true,
          target: (target ?? 0) / 52,
        );
        break;
      case Period.MONTHLY:
        graph = MonthDataChart(
          _createMonthlySeriesList(this.monthlyData!),
          animate: true,
          month: this.monthlyData == []
              ? this.monthlyData![0].date
              : DateTime.now(),
          target: (target ?? 0) / 12,
        );
        break;
      case Period.YEARLY:
        graph = YearDataChart(
          _createYearlySeriesList(yearlyData!),
          animate: true,
          target: target,
        );
        break;
